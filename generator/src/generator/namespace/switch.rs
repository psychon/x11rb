...
pub(super) fn emit_switch_type(
    ...
    out.indented(|out| {
        ...
        out.indented(|out| match switch.kind {
            ...
            xcbdefs::SwitchKind::Case => {
                ...
                out.indented(|out| {
                    for (idx, (case, case_info)) in
                        switch.cases.iter().zip(case_infos.iter()).enumerate()
                    {
                        let field_name = match case_info {
                            CaseInfo::SingleField(index) => {
                                case.fields.borrow()[*index].name().unwrap().to_string()
                        ...
                        outln!(
                            out,
                            "{idx} => Self::{}(crate::x11_utils::GenerateRandom::generate(rng)),",
                            to_rust_type_name(&field_name)
                        );
                    }
