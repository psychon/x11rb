...
impl TryFrom<XprotoImageOrder> for ImageOrder {
    ...
    fn try_from(value: XprotoImageOrder) -> Result<Self, ParseError> {
        match value {
            XprotoImageOrder::LSB_FIRST => Ok(Self::LeastSignificantByteFirst),
            XprotoImageOrder::MSB_FIRST => Ok(Self::MostSignificantByteFirst),
            _ => Err(ParseError::InvalidValue),
        }
    }
}
    ...
    pub fn put_pixel(&mut self, x: u16, y: u16, pixel: u32) {
        ...
        match self.bits_per_pixel {
            ...
            BitsPerPixel::B4 => {
                ...
                let odd_x = x % 2 == 1;
                let mask = if odd_x == (self.byte_order == ImageOrder::MostSignificantByteFirst) {
                    pixel <<= 4;
                    0xf0
                } else {
                    0x0f
            ...
            BitsPerPixel::B16 => {
                let (p0, p1) = match self.byte_order {
                    ImageOrder::LeastSignificantByteFirst => (pixel, pixel >> 8),
                    ImageOrder::MostSignificantByteFirst => (pixel >> 8, pixel),
                };
            ...
            BitsPerPixel::B24 => {
                let (p0, p1, p2) = match self.byte_order {
                    ImageOrder::LeastSignificantByteFirst => (pixel, pixel >> 8, pixel >> 16),
                    ImageOrder::MostSignificantByteFirst => (pixel >> 16, pixel >> 8, pixel),
                };
    ...
    pub fn get_pixel(&self, x: u16, y: u16) -> u32 {
        ...
        match self.bits_per_pixel {
            ...
            BitsPerPixel::B4 => {
                ...
                let odd_x = x % 2 == 1;
                if odd_x == (self.byte_order == ImageOrder::MostSignificantByteFirst) {
                    byte >> 4
                } else {
                    byte & 0x0f
                }
            ...
            BitsPerPixel::B16 => {
                ...
                match self.byte_order {
                    ImageOrder::LeastSignificantByteFirst => p0 | (p1 << 8),
                    ImageOrder::MostSignificantByteFirst => p1 | (p0 << 8),
                }
            }
            BitsPerPixel::B24 => {
                ...
                match self.byte_order {
                    ImageOrder::LeastSignificantByteFirst => p0 | (p1 << 8) | (p2 << 16),
                    ImageOrder::MostSignificantByteFirst => p2 | (p1 << 8) | (p0 << 16),
                }
            }
            BitsPerPixel::B32 => {
...
fn compute_depth_1_address(x: usize, order: ImageOrder) -> (usize, usize) {
    let bit = match order {
        ImageOrder::MostSignificantByteFirst => 7 - x % 8,
        ImageOrder::LeastSignificantByteFirst => x % 8,
    };
    (x / 8, bit)
}
...
mod test_image {
    ...
    fn test_new() {
        ...
        assert_eq!(image.byte_order(), ImageOrder::MostSignificantByteFirst);
