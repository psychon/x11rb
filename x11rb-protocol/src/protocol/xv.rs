...
#[cfg(test)]
impl crate::x11_utils::GenRandom for Type {
...
#[cfg(test)]
impl crate::x11_utils::GenRandom for ImageFormatInfoType {
    fn generate(rng: &fastrand::Rng) -> Self {
        let possible_values = &[
            Self::RGB.0,
            Self::YUV.0,
...
#[cfg(test)]
impl crate::x11_utils::GenRandom for ImageFormatInfoFormat {
    fn generate(rng: &fastrand::Rng) -> Self {
        let possible_values = &[
            Self::PACKED.0,
            Self::PLANAR.0,
...
#[cfg(test)]
impl crate::x11_utils::GenRandom for AttributeFlag {
    fn generate(rng: &fastrand::Rng) -> Self {
        let possible_values = &[
            Self::GETTABLE.0,
            Self::SETTABLE.0,
...
#[cfg(test)]
impl crate::x11_utils::GenRandom for VideoNotifyReason {
    fn generate(rng: &fastrand::Rng) -> Self {
        let possible_values = &[
            Self::STARTED.0,
            Self::STOPPED.0,
...
#[cfg(test)]
impl crate::x11_utils::GenRandom for ScanlineOrder {
    fn generate(rng: &fastrand::Rng) -> Self {
        let possible_values = &[
...
#[cfg(test)]
impl crate::x11_utils::GenRandom for GrabPortStatus {
    fn generate(rng: &fastrand::Rng) -> Self {
        let possible_values = &[
            Self::SUCCESS.0,
            Self::BAD_EXTENSION.0,
...
mod rational {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for Rational {
        fn generate(rng: &Rng) -> Self {
            let numerator: i32 = GenRandom::generate(rng);
            let denominator: i32 = GenRandom::generate(rng);
            Self {
                numerator,
                denominator,
            }
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(12392074869451200);
        let value = Rational::generate(&rng);
...
mod adaptor_info {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for AdaptorInfo {
        fn generate(rng: &Rng) -> Self {
            let num_formats = u32::from(rng.u8(..16));
            let name_size = u32::from(rng.u8(..16));
            let base_id: Port = GenRandom::generate(rng);
            let num_ports: u16 = GenRandom::generate(rng);
            let type_: u8 = GenRandom::generate(rng);
            let name: Vec<u8> = gen_random_list(rng, usize::try_from(name_size).unwrap());
            let formats: Vec<Format> = gen_random_list(rng, usize::try_from(num_formats).unwrap());
            Self {
                base_id,
                num_ports,
                type_,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(15994550724676515840);
        let value = AdaptorInfo::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod encoding_info {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for EncodingInfo {
        fn generate(rng: &Rng) -> Self {
            let name_size = u32::from(rng.u8(..16));
            let encoding: Encoding = GenRandom::generate(rng);
            let width: u16 = GenRandom::generate(rng);
            let height: u16 = GenRandom::generate(rng);
            let rate: Rational = GenRandom::generate(rng);
            let name: Vec<u8> = gen_random_list(rng, usize::try_from(name_size).unwrap());
            Self {
                encoding,
                width,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(5461725354066322752);
        let value = EncodingInfo::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod image {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for Image {
        fn generate(rng: &Rng) -> Self {
            let num_planes = u32::from(rng.u8(..16));
            let data_size = u32::from(rng.u8(..16));
            let id: u32 = GenRandom::generate(rng);
            let width: u16 = GenRandom::generate(rng);
            let height: u16 = GenRandom::generate(rng);
            let pitches: Vec<u32> = gen_random_list(rng, usize::try_from(num_planes).unwrap());
            let offsets: Vec<u32> = gen_random_list(rng, usize::try_from(num_planes).unwrap());
            let data: Vec<u8> = gen_random_list(rng, usize::try_from(data_size).unwrap());
            Self {
                id,
                width,
                height,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(8029337087);
        let value = Image::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod attribute_info {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for AttributeInfo {
        fn generate(rng: &Rng) -> Self {
            let size = u32::from(rng.u8(..16));
            let flags: u32 = GenRandom::generate(rng);
            let min: i32 = GenRandom::generate(rng);
            let max: i32 = GenRandom::generate(rng);
            let name: Vec<u8> = gen_random_list(rng, usize::try_from(size).unwrap());
            Self {
                flags,
                min,
                max,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(4598280336941095936);
        let value = AttributeInfo::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod image_format_info {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for ImageFormatInfo {
        fn generate(rng: &Rng) -> Self {
            let id: u32 = GenRandom::generate(rng);
            let type_: ImageFormatInfoType = GenRandom::generate(rng);
            let byte_order: xproto::ImageOrder = GenRandom::generate(rng);
            let guid: [u8; 16] = gen_random_list(rng, usize::try_from(16u32).unwrap());
            let bpp: u8 = GenRandom::generate(rng);
            let num_planes: u8 = GenRandom::generate(rng);
            let depth: u8 = GenRandom::generate(rng);
            let red_mask: u32 = GenRandom::generate(rng);
            let green_mask: u32 = GenRandom::generate(rng);
            let blue_mask: u32 = GenRandom::generate(rng);
            let format: ImageFormatInfoFormat = GenRandom::generate(rng);
            let y_sample_bits: u32 = GenRandom::generate(rng);
            let u_sample_bits: u32 = GenRandom::generate(rng);
            let v_sample_bits: u32 = GenRandom::generate(rng);
            let vhorz_y_period: u32 = GenRandom::generate(rng);
            let vhorz_u_period: u32 = GenRandom::generate(rng);
            let vhorz_v_period: u32 = GenRandom::generate(rng);
            let vvert_y_period: u32 = GenRandom::generate(rng);
            let vvert_u_period: u32 = GenRandom::generate(rng);
            let vvert_v_period: u32 = GenRandom::generate(rng);
            let vcomp_order: [u8; 32] = gen_random_list(rng, usize::try_from(32u32).unwrap());
            let vscanline_order: ScanlineOrder = GenRandom::generate(rng);
            Self {
                id,
                type_,
                byte_order,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(14966687519825487424);
        let value = ImageFormatInfo::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod video_notify_event {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for VideoNotifyEvent {
        fn generate(rng: &Rng) -> Self {
            let response_type: u8 = GenRandom::generate(rng);
            let reason: VideoNotifyReason = GenRandom::generate(rng);
            ...
            let port: Port = GenRandom::generate(rng);
            Self {
                response_type,
                reason,
                sequence,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(1229575400323856384);
        let value = VideoNotifyEvent::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod port_notify_event {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for PortNotifyEvent {
        fn generate(rng: &Rng) -> Self {
            let response_type: u8 = GenRandom::generate(rng);
            let sequence: u16 = GenRandom::generate(rng);
            let time: xproto::Timestamp = GenRandom::generate(rng);
            let port: Port = GenRandom::generate(rng);
            let attribute: xproto::Atom = GenRandom::generate(rng);
            let value: i32 = GenRandom::generate(rng);
            Self {
                response_type,
                sequence,
                time,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(15709037164957564928);
        let value = PortNotifyEvent::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod query_adaptors_reply {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for QueryAdaptorsReply {
        fn generate(rng: &Rng) -> Self {
            let num_adaptors = u32::from(rng.u8(..16));
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let info: Vec<AdaptorInfo> = gen_random_list(rng, usize::try_from(num_adaptors).unwrap());
            Self {
                sequence,
                length,
                info,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(7753652967096320000);
        let value = QueryAdaptorsReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod query_encodings_reply {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for QueryEncodingsReply {
        fn generate(rng: &Rng) -> Self {
            let num_encodings = u32::from(rng.u8(..16));
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let info: Vec<EncodingInfo> = gen_random_list(rng, usize::try_from(num_encodings).unwrap());
            Self {
                sequence,
                length,
                info,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(9202375974549377024);
        let value = QueryEncodingsReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod grab_port_reply {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for GrabPortReply {
        fn generate(rng: &Rng) -> Self {
            let result: GrabPortStatus = GenRandom::generate(rng);
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            Self {
                result,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(14454556895040045056);
        let value = GrabPortReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod query_best_size_reply {
    ...
    impl GenRandom for QueryBestSizeReply {
        fn generate(rng: &Rng) -> Self {
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let actual_width: u16 = GenRandom::generate(rng);
            let actual_height: u16 = GenRandom::generate(rng);
            Self {
                sequence,
                length,
                actual_width,
...
mod get_port_attribute_reply {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for GetPortAttributeReply {
        fn generate(rng: &Rng) -> Self {
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let value: i32 = GenRandom::generate(rng);
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(16573127677674782720);
        let value = GetPortAttributeReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
#[cfg(test)]
mod query_port_attributes_reply {
    #![allow(dead_code, unused_imports, clippy::useless_conversion)]
    use super::*;
    use crate::x11_utils::{GenRandom, Serialize, gen_random_list};
    use alloc::vec::Vec;
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for QueryPortAttributesReply {
        fn generate(rng: &Rng) -> Self {
            let num_attributes = u32::from(rng.u8(..16));
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let text_size: u32 = GenRandom::generate(rng);
            let attributes: Vec<AttributeInfo> = gen_random_list(rng, usize::try_from(num_attributes).unwrap());
            Self {
                sequence,
                length,
                text_size,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(17306596376099946496);
        let value = QueryPortAttributesReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
mod list_image_formats_reply {
    ...
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for ListImageFormatsReply {
        fn generate(rng: &Rng) -> Self {
            let num_formats = u32::from(rng.u8(..16));
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let format: Vec<ImageFormatInfo> = gen_random_list(rng, usize::try_from(num_formats).unwrap());
            Self {
                sequence,
                length,
                format,
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(5687026826181509120);
        let value = ListImageFormatsReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
...
#[cfg(test)]
mod query_image_attributes_reply {
    #![allow(dead_code, unused_imports, clippy::useless_conversion)]
    use super::*;
    use crate::x11_utils::{GenRandom, Serialize, gen_random_list};
    use alloc::vec::Vec;
    use core::convert::TryFrom;
    use fastrand::Rng;
    impl GenRandom for QueryImageAttributesReply {
        fn generate(rng: &Rng) -> Self {
            let num_planes = u32::from(rng.u8(..16));
            let sequence: u16 = GenRandom::generate(rng);
            let length: u32 = GenRandom::generate(rng);
            let data_size: u32 = GenRandom::generate(rng);
            let width: u16 = GenRandom::generate(rng);
            let height: u16 = GenRandom::generate(rng);
            let pitches: Vec<u32> = gen_random_list(rng, usize::try_from(num_planes).unwrap());
            let offsets: Vec<u32> = gen_random_list(rng, usize::try_from(num_planes).unwrap());
    ...
    #[test]
    fn check_serialize() {
        let rng = Rng::with_seed(1599671725766803456);
        let value = QueryImageAttributesReply::generate(&rng);
        let left = value.serialize();
        let mut right = alloc::vec![];
        value.serialize_into(&mut right);
        assert_eq!(&left[..], right.as_slice());
